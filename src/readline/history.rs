use std::{cmp::Ordering, env, path::PathBuf, sync::Arc, time::{Duration, SystemTime, UNIX_EPOCH}};

use rusqlite::Connection;

use crate::{libsh::error::{ShResult}, readline::{complete::{Candidate, FuzzySelector}, editcmd::Direction, linebuf::LineBuf}, state::read_shopts};
...
impl History {
	...
	pub fn new(table: &str) -> ShResult<Self> {
		...
		let db_path = PathBuf::from(env::var("SHED_HISTDB").unwrap_or_else(|_| {
			...
			dirs::data_dir().map(|p| p.to_string_lossy().to_string()).unwrap_or_else(|| format!("{home}/.local/share"))
		})).join("shed").join("shed_hist.db");
	...
	pub fn push(&self, command: String) -> ShResult<()> {
		...
		if read_shopts(|o| o.core.hist_ignore_dupes) {
			let last: Option<String> = self.conn.query_row(
				&format!("SELECT command FROM {} ORDER BY id DESC LIMIT 1", self.table),
				[],
				|row| row.get(0),
			).ok();
	...
	pub fn push_entry(&self, entry: HistEntry) -> ShResult<()> {
		let HistEntry { runtime, timestamp, command } = entry;
		if command.is_empty() {
			return Ok(());
		}
		if read_shopts(|o| o.core.hist_ignore_dupes) {
      ...
      && num != 0
    {
      match num.cmp(&0) {
				...
				Ordering::Less => {
					...
					self.conn.query_row(
						&format!("SELECT command FROM {} ORDER BY id DESC LIMIT 1 OFFSET ?1", self.table),
				...
				Ordering::Greater => {
					...
					self.conn.query_row(
						&format!("SELECT command FROM {} ORDER BY id ASC LIMIT 1 OFFSET ?1", self.table),
						rusqlite::params![offset],
						|row| row.get(0),
					).ok()
				}
        _ => unreachable!(),
			...
			self.conn.query_row(
				&format!("SELECT command FROM {} WHERE command LIKE ?1 || '%' ORDER BY id DESC LIMIT 1", self.table),
	...
	pub fn last(&self) -> Option<HistEntry> {
		self.conn.query_row(
			&format!("SELECT command, timestamp, runtime FROM {} ORDER BY id DESC LIMIT 1", self.table),
			[],
			Self::row_to_entry
		).ok()
	}

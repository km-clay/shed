...
use std::{
  env, io::Write, ops::Range, path::{Path, PathBuf}, rc::Rc
};

use crate::{
  builtin::{help::{markup::StyledHelp, pager::{HelpPager, PagerCmd, PagerEvent}}, join_raw_arg_iter}, libsh::{error::ShResult, guards::{RawModeGuard, TuiGuard}, sys::TTY_FILENO}, parse::{
    NdRule, Node,
    execute::{exec_input, prepare_argv},
  }, procio::borrow_fd, readline::complete::ScoredCandidate, sherr, state::{self, write_meta}
};
...
pub fn get_help_content(topic: &str) -> Option<(usize,String,Option<String>)> {
  ...
  log::debug!("tags: {tags:#?}");
    ...
    let file_name = file.file_stem().map(|s| s.to_string_lossy().to_string());
		std::fs::read_to_string(file).ok().map(|content| (line.saturating_sub(2),content,file_name))
	})
}
...
pub fn open_help(content: &str, line: usize, filename: Option<String>) -> ShResult<()> {
	...
	loop {
		...
		match poll(&mut [tty_fd], PollTimeout::NONE) {
			...
			Err(e) => return Err(sherr!(InternalErr, "Error polling for help pager input: {e}")),
		}
...
pub fn read_tags(path: &Path) -> ShResult<Vec<ScoredTag>> {
  ...
  let tags = styled.find_markers(TAG_SEQ)
		.into_iter()
		.map(|span| ScoredTag::new(

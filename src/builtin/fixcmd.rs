		...
		match word.as_str() {
			...
			"-e" => {
				let Some((word,_)) = words_iter.next() else {
					return Err(ShErr::at(ShErrKind::ParseErr, span, "Option -e requires an argument"));
				};
...
fn fc_edit(hist: History, opts: FixCmdOpts, interactive: bool) -> ShResult<()> {
	let editor = if let Some(editor) = opts.editor {
		...
		return Err(ShErr::simple(ShErrKind::ExecFail, "No editor specified for fc command"));
	};
	...
	for (_, entry) in entries {
		...
		exec_input(new_cmd.clone(), None, interactive, Some("fc re-exec".into()))?;
...
fn fc_list(hist: History, opts: FixCmdOpts) -> ShResult<()> {
	let first = if let Some(first) = opts.first { first } else {
		RangeArg::Number(-16)
	};
...
fn get_entry_range(
	...
	let resolve = |arg: &RangeArg| -> i64 {
		match arg {
			...
			RangeArg::Prefix(p) => {
				hist.query_by_prefix(p).map(|(id, _)| id).unwrap_or(last_id)
			}
		}
	};

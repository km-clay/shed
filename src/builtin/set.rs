...
use crate::{
  libsh::{error::{ShErr, ShErrKind, ShResult, ShResultExt}, utils::VecDequeExt}, parse::{NdRule, Node, execute::prepare_argv}, prelude::*, procio::borrow_fd, shopt::as_var_val_display, state::{self, VarKind, read_shopts, read_vars, write_shopts, write_vars}, two_way_display
};
...
impl TryFrom<char> for SetFlags {
	...
	fn try_from(value: char) -> Result<Self, Self::Error> {
	  ...
	  match value {
			...
			'v' => Ok(Self::VERBOSE),
			'x' => Ok(Self::XTRACE),
			_ => Err(ShErr::simple(ShErrKind::ParseErr, format!("invalid option: {}", value))),
		}
	}
}
...
impl FromStr for SetFlags {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
	  match s {
			...
			_ => Err(ShErr::simple(ShErrKind::ParseErr, format!("invalid option: {}", s))),
		}
	}
}
...
pub fn build_set_call(readable: bool) -> String {
	...
	let opts = write_shopts(|o| {
		o.query("set").unwrap().unwrap()
	});
	if !readable {
		let mut call = String::from("set ");

		let lines: Vec<_> = opts.lines()
			.map(|l| {
				l.strip_prefix("set.")
					.unwrap()
					.split_once('=')
		...
		let on: Vec<&str> = lines.iter()
			.filter(|(_, r)| *r == "true")
			.map(|(l, _)| *l)
			.collect();

		let on_chars: String = on.iter()
			.filter_map(|opt| {
				SetFlags::from_str(opt)
					.unwrap()
					.as_char()
			})
			.collect();

		let on_strs: String = on.into_iter()
			.filter(|opt| {
				SetFlags::from_str(opt)
					.unwrap()
					.as_char()
		...
		let off: Vec<_> = lines.iter()
			.filter(|(_, r)| *r != "true").
			map(|(l, _)| *l).
			collect();

		let off_chars: String = off.iter()
			.filter_map(|opt| {
				SetFlags::from_str(opt)
					.unwrap()
					.as_char()
		...
		let off_strs: String = off.into_iter()
			.filter(|opt| {
				SetFlags::from_str(opt)
					.unwrap()
					.as_char()
		...
		match chars.peek() {
			Some(polarity @ ('+' | '-')) => {
				...
				for opt in flags.get_shopt_fields() {
					...
					write_shopts(|o| o.query(&format!("set.{opt}={opt_val}"))).promote_err(nd_span.clone())?;
				}
			}

...
impl HistQuery {
	...
	pub fn execute(&self, hist: &History) -> ShResult<Vec<(i64, HistEntry)>> {
		...
		if let Some(after) = &self.after {
			let ts = parse_date_string(after, Utc::now(), Dialect::Us)
				.map_err(|e| ShErr::simple(ShErrKind::ParseErr, format!("Failed to parse date for --after: {e}")))?;
		...
		if let Some(before) = &self.before {
			let ts = parse_date_string(before, Utc::now(), Dialect::Us)
				.map_err(|e| ShErr::simple(ShErrKind::ParseErr, format!("Failed to parse date for --before: {e}")))?;
			conditions.push(format!("timestamp <= ?{idx}"));
			params.push(Box::new(ts.timestamp()));
			idx += 1;
		}
		...
		if let Some(duration) = &self.longer_than {
			let secs = chrono_english::parse_duration(duration)
				.map_err(|e| ShErr::simple(ShErrKind::ParseErr, format!("Failed to parse duration for --longer-than: {e}")))?;
		...
		if let Some(duration) = &self.shorter_than {
			let secs = chrono_english::parse_duration(duration)
				.map_err(|e| ShErr::simple(ShErrKind::ParseErr, format!("Failed to parse duration for --shorter-than: {e}")))?;
			conditions.push(format!("runtime <= ?{idx}"));
			match secs {
				Interval::Seconds(n) => {
					let dur = Duration::from_secs(n as u64).as_micros();
		...
		if let Some(pat) = &self.matches {
			match Regex::new(pat) {
				Ok(r) => {
					Ok(entries.into_iter().filter(|e| r.is_match(e.1.command())).collect())
				}
				Err(e) => {
					Err(ShErr::simple(ShErrKind::ParseErr, format!("Invalid regex for --matches: {e}")))
				}
			}
		} else {
			Ok(entries)
	...
	pub fn from_opts(opts: &[Opt]) -> ShResult<Self> {
		...
		for opt in opts {
			match opt {
				...
				_ => {
					return Err(ShErr::simple(ShErrKind::ParseErr, format!("Unknown option for history: {opt}")));
				}
			}
		}
	...
	pub fn format_entries(&self, entries: &[(i64, HistEntry)]) -> String {
		if self.json {
			let json: serde_json::Value = serde_json::Value::Object(entries.iter()
				.map(|e| {
					let HistEntry { runtime, timestamp, command } = &e.1;
					let mut map = serde_json::Map::new();
					map.insert("runtime".into(), serde_json::Value::Number((runtime.as_micros() as i64).into()));
					map.insert("timestamp".into(), serde_json::Value::Number((timestamp.duration_since(UNIX_EPOCH).unwrap().as_secs()).into()));
...
pub fn hist_builtin(node: Node) -> ShResult<()> {
  ...
  let (_argv, opts) = get_opts_from_tokens(argv, &HistQuery::opt_spec()).promote_err(span.clone())?;
